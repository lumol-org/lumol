...
impl Output for StressOutput {
    fn setup(&mut self, _: &System) {
        ...
        if let Err(err) = writeln!(&mut self.file, "# step stress.xx stress.yy stress.zz stress.xy stress.xz stress.yz") {
            fatal_error!("Could not write to file '{}': {}", self.path.display(), err);
        }
    }
    ...
    fn write(&mut self, system: &System) {
        ...
        if let Err(err) = writeln!(&mut self.file, "{} {} {} {} {} {} {}", system.step(), xx, yy, zz, xy, xz, yz) {
...
mod tests {
    ...
    fn energy() {
        test_output(|path| {
            Box::new(StressOutput::new(path).unwrap())
        },
