...
pub fn table<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a Table> {
    let table = try!(
        config.get(key)
              .ok_or(Error::from(format!("Missing '{}' key in {}", key, context)))
    );
    return table.as_table()
                .ok_or(Error::from(format!("'{}' must be a table in {}", key, context)));
}
...
pub fn str<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a str> {
    ...
    return string.as_str()
                 .ok_or(Error::from(format!("'{}' must be a string in {}", key, context)));
}
...
pub fn slice<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a [Value]> {
    ...
    let array = array.as_array()
                     .ok_or(Error::from(format!("'{}' must be an array in {}", key, context)));
...
pub fn typ<'a>(config: &'a Table, context: &str) -> Result<&'a str> {
    let typ = try!(
        config.get("type")
              .ok_or(Error::from(format!("Missing 'type' key in {}", context)))
    );
